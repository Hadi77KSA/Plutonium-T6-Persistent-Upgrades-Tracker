...
init()
{
    ...
    CreateCommand( level.chat_commands[ "ports" ], "toggletrackerhuddetails" , "function", ::tracker_hud_details_command );
...
toggle_tracker_hud( toggle )
{
    ...
    {
        return array( "Invalid input", GetDvar("cc_prefix") + "toggletrackerhud" + " only accepts:", "'1' or 'on' to enable", "'0' or 'off' to disable" );
    }
...
toggle_tracker_hud_details( variable_to_toggle , toggle )
{
    ...
    {
        error = array( "Invalid input", GetDvar("cc_prefix") + "toggletrackerhuddetails" + " could not find " + toLower( variable_to_toggle ) );
        return error;
    }
    
    if( !custom_is_in_array( toLower( toggle ), array( "1", "on", "0", "off" ) ) )
    {
        error = array( "Invalid input", GetDvar("cc_prefix") + "toggletrackerhuddetails" + " " + toLower( variable_to_toggle ) + " only accepts:", "'1' or 'on' to enable", "'0' or 'off' to disable" );
        return error;
    }
    else if( custom_is_in_array( toLower( toggle ), array( "0", "off" ) ) )
    {
